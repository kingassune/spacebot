...
/// All Prometheus metric handles for the James process.
///
...
impl Metrics {
    fn new() -> Self {
        ...
        let llm_requests_total = IntCounterVec::new(
            Opts::new("james_llm_requests_total", "Total LLM completion requests"),
            &["agent_id", "model", "tier"],
        )
        .expect("hardcoded metric descriptor");

        let tool_calls_total = IntCounterVec::new(
            Opts::new("james_tool_calls_total", "Total tool calls executed"),
            &["agent_id", "tool_name"],
        )
        .expect("hardcoded metric descriptor");

        let memory_reads_total =
            IntCounter::new("james_memory_reads_total", "Total memory recall operations")
                .expect("hardcoded metric descriptor");

        let memory_writes_total =
            IntCounter::new("james_memory_writes_total", "Total memory save operations")
                .expect("hardcoded metric descriptor");
        ...
        let active_workers = IntGaugeVec::new(
            Opts::new("james_active_workers", "Currently active workers"),
            &["agent_id"],
        )
        .expect("hardcoded metric descriptor");

        let memory_entry_count = IntGaugeVec::new(
            Opts::new("james_memory_entry_count", "Total memory entries per agent"),
            &["agent_id"],
        )
        .expect("hardcoded metric descriptor");

        let llm_tokens_total = IntCounterVec::new(
            Opts::new("james_llm_tokens_total", "Total LLM tokens consumed"),
        ...
        let active_branches = IntGaugeVec::new(
            Opts::new("james_active_branches", "Currently active branches"),
            &["agent_id"],
        )
        .expect("hardcoded metric descriptor");
        ...
        let process_errors_total = IntCounterVec::new(
            Opts::new("james_process_errors_total", "Process errors by type"),
            &["agent_id", "process_type", "error_type"],
        )
        .expect("hardcoded metric descriptor");

        let memory_updates_total = IntCounterVec::new(
            Opts::new("james_memory_updates_total", "Memory mutation operations"),
            &["agent_id", "operation"],
        )
        .expect("hardcoded metric descriptor");
        ...
        let dispatch_while_cold_count = IntCounterVec::new(
            Opts::new(
                "james_dispatch_while_cold_count",
        ...
        let warmup_recovery_latency_ms = HistogramVec::new(
            HistogramOpts::new(
                "james_warmup_recovery_latency_ms",
