...
Comprehensive reference for James's Prometheus metrics. For quick-start setup, see `docs/metrics.md`. For the published docs, see the metrics page on [docs.james.sh](https://docs.james.sh).
...
All metrics are prefixed with `james_`. The registry uses a private `prometheus::Registry` (not the default global one) to avoid conflicts with other libraries.

### Counters

#### `james_llm_requests_total`

| Field | Value |
|-------|-------|
| Type | `IntCounterVec` |
| Labels | `agent_id`, `model`, `tier` |
| Instrumented in | `src/llm/model.rs` — `JamesModel::completion()` |
...
#### `james_tool_calls_total`

| Field | Value |
|-------|-------|
| Type | `IntCounterVec` |
| Labels | `agent_id`, `tool_name` |
| Instrumented in | `src/hooks/james.rs` — `JamesHook::on_tool_result()` |
...
#### `james_llm_estimated_cost_dollars`
...
**Cardinality:** Same as `james_llm_requests_total`.
...
#### `james_process_errors_total`

| Field | Value |
|-------|-------|
| Type | `IntCounterVec` |
| Labels | `agent_id`, `process_type`, `error_type` |
| Instrumented in | `src/llm/model.rs` — `JamesModel::completion()` error paths |
...
#### `james_llm_request_duration_seconds`
...
**Cardinality:** Same as `james_llm_requests_total` (per-bucket overhead is fixed, not per-series).

#### `james_tool_call_duration_seconds`

| Field | Value |
|-------|-------|
| Type | `Histogram` (no labels) |
| Buckets | 0.01, 0.05, 0.1, 0.25, 0.5, 1, 2.5, 5, 10, 30 |
| Instrumented in | `src/hooks/james.rs` — `on_tool_call()` starts timer, `on_tool_result()` observes |
...
| `src/hooks/james.rs` | `#[cfg(feature = "metrics")] static TOOL_CALL_TIMERS` + 2 blocks |
