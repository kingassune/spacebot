...
//!
//! Runs against the real ~/.james data directory. Requires:
//! - ~/.james/config.toml with valid LLM credentials
...
/// Bootstrap an AgentDeps from the real ~/.james config, using the first
/// (default) agent's databases and config.
async fn bootstrap_deps() -> anyhow::Result<james::AgentDeps> {
...
fn test_bulletin_prompts_cover_all_memory_types() {
    ...
    for memory_type in james::memory::types::MemoryType::ALL {
    ...
    assert_eq!(
        ...
        james::memory::types::MemoryType::ALL.len(),
...
async fn test_memory_recall_returns_results() {
    ...
    let config = james::memory::search::SearchConfig::default();
    let results = deps
        .memory_search
...
async fn test_bulletin_generation() {
    ...
    let success = james::agent::cortex::generate_bulletin(&deps, &logger).await;
