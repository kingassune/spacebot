...
How James gives LLM processes the ability to act.
...
James uses four ToolServer configurations:
...
Leak detection (via `JamesHook`) scans all tool output for secret patterns (API keys, tokens, PEM keys) and terminates the process if a leak is found. This includes base64-encoded, URL-encoded, and hex-encoded variants.
