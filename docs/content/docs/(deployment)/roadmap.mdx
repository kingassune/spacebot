---
title: Roadmap
description: Development timeline and future plans for James.
---
...
- **LLM** — `JamesModel` implements Rig's `CompletionModel`, routes through `LlmManager` via HTTP with retries and fallback chains across 13 providers (Anthropic, OpenAI, OpenRouter, Kilo Gateway, Z.ai, Groq, Together, Fireworks, DeepSeek, xAI, Mistral, OpenCode Zen, OpenCode Go)
...
- **Hooks** — `JamesHook` with tool call/result events, leak detection (11 patterns scanning both tool args and output); `CortexHook` for observation (trace-only)
...
- **Agent CLI** — `james agents list/create/delete`, identity template bootstrapping
...
Implement `JamesModel.stream()` with SSE parsing. The messaging adapters already handle `StreamStart`/`StreamChunk`/`StreamEnd` response types — this is purely the LLM layer.
...
- Tool nudging — inject "use your tools" in `JamesHook.on_completion_response()` when LLM responds with text in early iterations
...
- `james setup --quick --provider openrouter --key sk-...` — non-interactive fast path for scripted deploys
- `james setup --channels-only` — add channels to an existing instance

## Decided Against

### Built-in Tunnel Support

Some self-hosted agent frameworks bundle Cloudflare/ngrok/Tailscale tunnel management into the binary. We intentionally don't — it's unnecessary for James's architecture.
...
For hosted instances, the platform proxy handles access over a private WireGuard mesh. For self-hosted instances, the embedded API and UI are accessible on your LAN. If you want remote access, Tailscale works out of the box — just install it on the host machine and access the James API via your tailnet. No integration needed, no config, no child process management.
...
James handles this through its control interface and the cortex. The embedded UI shows adapter status, agent health, and active connections in real-time. The cortex observes system-wide signals and surfaces issues proactively. Channel token validation happens at setup time through the bindings API — if something is misconfigured, you find out immediately when you add the binding, not by running `doctor` ten minutes later.
