...
```bash
security find-generic-password -s "sh.james.master-key" -a "instance" -w
```

**Linux:**

The key is stored in the kernel keyring under the description `sh.james.master-key:instance`. Use `keyctl` to search for it:

```bash
keyctl search @s user "sh.james.master-key:instance"
...
The secrets database is instance-level -- a single store shared across all agents. It is separate from the main `james.db` SQLite database. It uses redb for single-writer, lock-free reads.

On first startup after upgrading from a per-agent store layout, the bootstrap process automatically migrates secrets from any legacy `~/.james/agents/{id}/data/secrets.redb` files into the instance-level store.
