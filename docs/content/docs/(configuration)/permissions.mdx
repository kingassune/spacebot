...
**OS-level containment is handled by the [Sandbox](/docs/sandbox) when sandboxing is enabled.** The sandbox enforces filesystem boundaries and environment sanitization at the kernel level, with exact guarantees depending on mode and backend/platform. On james.sh, each user also runs in an isolated container. The permissions system handles a different layer -- inter-agent boundaries and tool-level restrictions within a single James process.
...
```
~/.james/agents/{agent_id}/workspace/   # identity files, prompt overrides
~/.james/agents/{agent_id}/data/        # databases (read-only for file tool, managed by James)
~/.james/agents/{agent_id}/archives/    # compaction archives
```

But NOT:

```
~/.james/agents/{other_agent}/          # other agents' data
~/.james/config.toml                    # instance config (secret references, not plaintext keys)
...
During CLI onboarding (`james setup` or first run), the user picks a profile for each agent. Profiles set all permission values at once. Users can customize after.
...
**Interactive approval workflows.** No "approve this command?" prompts. James is a daemon without a UI. Permissions are declarative in config, not interactive at runtime. If a tool is allowed, it runs. If it's denied, it fails with an error.
