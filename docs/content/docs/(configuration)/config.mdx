...
Complete reference for `config.toml`. James looks for this file at `~/.james/config.toml` (or `$JAMES_DIR/config.toml`). If no config file exists, it falls back to environment variables.

## File Location

```
~/.james/config.toml          # default
$JAMES_DIR/config.toml        # env override
james --config /path/to.toml  # CLI override
```
...
base_url = "http://localhost:8080" # do not include /v1; James appends endpoint paths
...
id = "main"
default = true
workspace = "/custom/workspace/path"   # optional, defaults to ~/.james/agents/{id}/workspace
...
LLM keys also have implicit env fallbacks — if no key is set in the TOML, James checks `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `OPENROUTER_API_KEY`, `KILO_API_KEY`, and `OPENCODE_GO_API_KEY` automatically.
...
If no `config.toml` exists, James runs from environment variables alone:

```bash
export ANTHROPIC_API_KEY="sk-ant-..."
# optional model overrides
export JAMES_CHANNEL_MODEL="anthropic/claude-sonnet-4-20250514"
export JAMES_WORKER_MODEL="anthropic/claude-haiku-4.5-20250514"
james
```
...
Most config values are hot-reloaded when their files change. James watches `config.toml`, identity files, and skill directories. Changes are debounced to 2 seconds and applied to all running channels, workers, and branches without restart.
...
- `~/.james/config.toml`
- `~/.james/skills/` (instance-level skills)
- Each agent's `workspace/` (identity files: SOUL.md, IDENTITY.md, USER.md)
- Each agent's `workspace/skills/` (workspace-level skills)

On file change, James re-reads the changed files and atomically swaps the new values into the live `RuntimeConfig` using `arc-swap`. All consumers (channels, branches, workers, compactors, cron jobs) read from `RuntimeConfig` on every use, so they pick up changes immediately.
...
> - James appends the endpoint path automatically:
...
If no prefix is provided (for example `claude-sonnet-4-20250514`), James defaults to the `anthropic` provider.
...
When branch/worker/cron dispatch happens before readiness is satisfied, James still dispatches, increments cold-dispatch metrics, and queues a forced warmup pass in the background.
...
| `workspace` | string | `~/.james/agents/{id}/workspace` | Custom workspace path |
...
If a configured timezone is invalid, James logs a warning and falls back to server local time.

Channel/worker temporal context timezone precedence is:

1. `agents.user_timezone`
2. `defaults.user_timezone`
3. `JAMES_USER_TIMEZONE`
4. resolved cron timezone (from `agents.cron_timezone` / `defaults.cron_timezone` / `JAMES_CRON_TIMEZONE`)
