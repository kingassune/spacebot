...
Workers never see `ANTHROPIC_API_KEY`, `DISCORD_BOT_TOKEN`, `JAMES_*` internal vars, or any other environment variables from the parent process.
...
All tool output (shell, exec, file, browser) is scanned for known secret patterns before being returned to the LLM. This runs in the `JamesHook` after every tool execution.
...
| **Leak detection** | Regex scan of all tool output for secret patterns | All tools via JamesHook |
