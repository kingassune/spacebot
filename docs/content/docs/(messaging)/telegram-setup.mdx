...
description: Connect James to Telegram.
---

# Telegram Setup

Connect James to Telegram. Takes about 2 minutes.
...
- **Name**: The display name (e.g. "My James")
- **Username**: Must end in `bot` (e.g. `my_james_bot`)
...
- **Groups**: Add the bot to the group, send a message, and check the James logs — the `conversation_id` will show as `telegram:<chat_id>`. Group IDs are negative numbers (e.g. `-100123456789`).
