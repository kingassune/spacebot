---
title: Twitch Setup
description: Connect James to Twitch chat.
---

# Twitch Setup

Connect James to Twitch chat. Takes about 10 minutes.
...
## Step 4: Add Credentials to James

James needs your **username**, **OAuth token**, **client ID**, **client secret**, and **refresh token**. With all five, James automatically refreshes expired tokens.

<Tabs items={["James UI", "TOML Config"]}>
<Tab value="James UI">

1. Open your James dashboard
...
James connects immediately — no restart needed.
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `Login authentication failed` | Invalid or expired OAuth token | Re-run the token flow from Step 3. If you have `client_id`, `client_secret`, and `refresh_token` configured, James refreshes automatically — check that all three are set. |
