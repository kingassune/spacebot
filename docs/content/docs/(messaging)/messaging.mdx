---
title: Messaging
description: How James connects to Discord, Slack, Telegram, Twitch, Email, and webhooks.
---

# Messaging

James connects to chat platforms so your agent can talk to people where they already are. You can connect multiple platforms at once — messages from all of them flow into the same agent system.
...
2. James opens a persistent connection to that platform
3. When someone sends a message, James receives it and decides whether to reply, skip, or delegate work
...
You can run multiple agents on the same James instance, each connected to different places. Each agent has its own memory, personality, and conversation history.
...
Each chat context maps to its own James conversation with isolated history:
