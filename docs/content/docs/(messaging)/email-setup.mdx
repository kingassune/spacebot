---
title: Email Setup
description: Connect James to an inbox with IMAP + SMTP.
---

# Email Setup

Connect James to any mailbox that supports IMAP and SMTP. Inbound messages are polled over IMAP, and replies are sent over SMTP.
...
## Step 2: Add Email credentials to James
...
- James triages inbound mail first.
...
James keeps one conversation per email thread. It uses `References`, `In-Reply-To`, and `Message-ID` headers to map replies back to the correct conversation.
...
2. Confirm a new channel appears in James with a subject-based name.
...
| Bot ignores automated mails | Auto-response headers detected | Expected behavior; James skips auto-generated mail loops |
