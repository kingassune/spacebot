...
How James decides which LLM to use for each process.
...
Routing decisions in James are explicit, not inferred. We know the process type, the task type, and the purpose at spawn time. No keyword scoring, no LLM classifier, no content analysis.
...
let model = JamesModel::make(&llm_manager, model_name)
...
Fallback is built into `JamesModel::completion()`. When the primary model returns a retriable error:
...
match self.attempt_completion(request.clone()).await {
    ...
    Err(error) if is_retriable_error(&error) => {
        ...
        for fallback_name in routing.get_fallbacks(&self.full_model_name) {
            let fallback = JamesModel::make(&self.llm_manager, fallback_name);
