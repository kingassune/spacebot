...
The cortex is the system's awareness of itself. Every other process in James is focused on a specific job — channels talk to users, branches think, workers execute, compactors manage context. None of them see the whole picture. The cortex does.
...
If dispatch arrives while not ready, James does **not** block the channel or scheduler:
...
- `james_dispatch_while_cold_count{agent_id,dispatch_type,reason}`
- `james_warmup_recovery_latency_ms{agent_id,dispatch_type}`
