...
James is an agentic AI system where every LLM process has a dedicated role and delegation is the only way work gets done. It replaces the monolithic session model with specialized processes that only do one thing.
...
James runs as a single binary with no server dependencies. All data lives in embedded databases:
